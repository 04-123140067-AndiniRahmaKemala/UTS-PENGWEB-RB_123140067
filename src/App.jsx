...
import './styles/App.css'; // Path CSS yang benar

function App() {
  // Semua State harus didefinisikan di awal fungsi
  const [results, setResults] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [searchTerm, setSearchTerm] = useState('jack johnson');
  const [mediaType, setMediaType] = useState('music');
  const [sortBy, setSortBy] = useState('releaseDate'); // State sortBy harus ada!
  
  // State Playlist (MENGATASI ReferenceError: playlist is not defined)
  ...
  // Fetch API Logic (async/await, API Integration)
  ...
  const sortedResults = [...results].sort((a, b) => { 
    ...
    return new Date(b.releaseDate) - new Date(a.date);
  });
